...
| 2026-06-28 | 外部 FastAPI 项目先记录已验证运行关系，当前不迁移代码 | 现有本机环境可启动并通过测试，但直接迁移会同时涉及状态文件、虚拟环境、路径和秘密信息风险 | 新增 `docs/external_fastapi_delivery.md`；明确四个路由、必需文件、环境变量、已验证启动和尚无完整可移植依赖清单的边界 |
| 2026-06-28 | Level1 总览视频只展示游戏本体，AI 守卫闭环留给独立视频 | 第一条视频需要先让观看者理解倒计时潜入、敌人压力和路线选择；同时展开 AI 对话会造成信息过载和素材重复 | 新增 `docs/level1_overview_recording_checklist.md`；`01` 控制在 60 到 90 秒，`02` 继续展示真情报到暗道入口的完整闭环 |
| 2026-06-30 | 暂缓 `01_level1_overview.mp4` 实际录制，保留清单但不伪造完成状态 | 用户明确选择跳过这一步；录屏不应阻塞后续开发，也不应用空文件代替真实素材 | 视频状态记为“暂缓”；下一阶段转向多 Agent 游戏世界最小原型范围确认 |
